...
int main(int argc, char *argv[]) {
    ...
    // Position
    {
        ...
        is_termlog("Stack saves/restores virtual cursor position", GOTO(2, 2), SETPEN(),
    ...
    // Clipping
    {
        ...
        {
            tickit_renderbuffer_save(rb);
            tickit_renderbuffer_clip(
                rb, &(TickitRect){.top = 0, .left = 2, .lines = 10, .cols = 16});
        ...
        tickit_renderbuffer_flush_to_term(rb, tt);
        is_termlog("Stack saves/restores clipping region", GOTO(0, 0), SETPEN(),
    ...
    // Pen
    {
        ...
        tickit_renderbuffer_setpen(rb, pen = tickit_pen_new_attrs(TICKIT_PEN_BG, 1, 0));
...

        {
            ...
            tickit_renderbuffer_setpen(rb, pen = tickit_pen_new_attrs(TICKIT_PEN_FG, 4, 0));
            tickit_pen_unref(pen);
        ...
        tickit_renderbuffer_flush_to_term(rb, tt);
        is_termlog("Stack saves/restores render pen", GOTO(3, 0), SETPEN(.bg = 1), PRINT("123"),
            SETPEN(.bg = 1, .fg = 4), PRINT("456"), SETPEN(), PRINT("789"), SETPEN(.bg = 1),
            PRINT("ABC"), NULL);

        tickit_renderbuffer_goto(rb, 4, 0);

        tickit_renderbuffer_setpen(rb, pen = tickit_pen_new_attrs(TICKIT_PEN_REVERSE, 1, 0));
        tickit_pen_unref(pen);
...

        {
            ...
            tickit_renderbuffer_setpen(rb, pen = tickit_pen_new_attrs(TICKIT_PEN_REVERSE, 0, 0));
            tickit_pen_unref(pen);
        ...
        tickit_renderbuffer_flush_to_term(rb, tt);
        is_termlog("Stack saves/restores allow zeroing pen attributes", GOTO(4, 0), SETPEN(.rv = 1),
            PRINT("123"), SETPEN(), PRINT("456"), SETPEN(.rv = 1), PRINT("789"), NULL);
    }
    ...
    // Translation
    {
        ...
        tickit_renderbuffer_flush_to_term(rb, tt);
        is_termlog("Stack saves/restores translation offset", GOTO(0, 0), SETPEN(), PRINT("A"),
            GOTO(2, 2), SETPEN(), PRINT("C"), GOTO(3, 3), SETPEN(), PRINT("B"), NULL);
    }
