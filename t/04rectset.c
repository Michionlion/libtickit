...
void test_add(const char *input, const char *output) {
    ...
    for (reverse = 0; reverse < 2; reverse++) {
        ...
        for (i = 0; i < n_exp_outputs; i++) {
            ...
            if (got->top != exp->top || got->left != exp->left || got->lines != exp->lines ||
                got->cols != exp->cols) {
                diag("got %d,%d..%d,%d expected %d,%d..%d,%d", got->left, got->top,
                    tickit_rect_right(got), tickit_rect_bottom(got), exp->left, exp->top,
                    tickit_rect_right(exp), tickit_rect_bottom(exp));
                goto fail_dir;
            }
        }
...
int main(int argc, char *argv[]) {
    {
        ...
        is_int(tickit_rectset_get_rect(trs, 0, &tmp), 1,
            "tickit_rectset_get_rect after tickit_rectset_Add");
        is_rect(&tmp, "10,10+20,5", "tmp after tickit_rectset_get_rect");

        is_int(tickit_rectset_get_rects(trs, rects, 4), 1,
            "tickit_rectset_get_rects after tickit_rectset_add");
        is_rect(rects + 0, "10,10+20,5", "rects[0] after tickit_rectset_add");

        tickit_rectset_add(trs, rect_init_strp(&tmp, "10,20+20,2"));

        is_int(tickit_rectset_get_rects(trs, rects, 4), 2,
            "tickit_rectset_get_rects after second tickit_rectset_add");
        ...
        is_int(tickit_rectset_get_rects(trs, rects, 4), 2,
            "tickit_rectset_get_rects after second tickit_rectset_translate");
        ...
        ok(!tickit_rectset_intersects(trs, rect_init_strp(&tmp, "15,6..25,9")),
            "doesn't intersect no overlap");
        ...
        ok(!tickit_rectset_contains(trs, rect_init_strp(&tmp, "5,2..12,9")),
            "doesn't contain split");
        ok(!tickit_rectset_contains(trs, rect_init_strp(&tmp, "15,6..25,9")),
            "doesn't contain non-intersect");
