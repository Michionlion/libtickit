...
int main(int argc, char *argv[]) {
    ...
    // Basics
    {
        ...
        is_termlog("Termlog after focus", GOTO(3, 10), NULL);
        ...
        is_termlog("Termlog after window reposition", GOTO(5, 15), NULL);

        tickit_window_set_cursor_position(win, 2, 2);
        tickit_window_flush(root);

        is_termlog("Termlog after set cursor position", GOTO(7, 17), NULL);

        tickit_window_set_cursor_shape(win, TICKIT_CURSORSHAPE_UNDER);
        tickit_window_flush(root);

        is_termlog("Termlog after cursor_shape", GOTO(7, 17), NULL);
        ...
        is_termlog("Termlog empty after cursor_visible false", NULL);
        ...
        is_termlog("Termlog after focus window show", GOTO(7, 17), NULL);
    }
...

    {
        ...
        is_termlog("Termlog after winA takes focus", GOTO(5, 0), NULL);
        ...
        is_termlog("Termlog after winB takes focus", GOTO(6, 0), NULL);

        tickit_window_hide(winB);
        tickit_window_take_focus(winA);
        tickit_window_flush(root);

        is_termlog("Termlog after winB hidden", GOTO(5, 0), NULL);

        tickit_window_hide(winA);
        tickit_window_show(winB);
        tickit_window_flush(root);

        is_termlog("Termlog after winA hidden / winB shown", GOTO(6, 0), NULL);
    ...
    // Child notifications
    {
        ...
        int bind_id =
            tickit_window_bind_event(win, TICKIT_WINDOW_ON_FOCUS, 0, &on_focus_push, NULL);
