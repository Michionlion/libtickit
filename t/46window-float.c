...
int main(int argc, char *argv[]) {
    ...
    {
        int bind_id =
            tickit_window_bind_event(root, TICKIT_WINDOW_ON_EXPOSE, 0, &on_expose_fillchr, "X");

        tickit_window_expose(root, &(TickitRect){.top = 10, .lines = 1, .left = 0, .cols = 80});
        tickit_window_flush(root);

        is_termlog("Termlog for print under floating window", GOTO(10, 0), SETPEN(),
            PRINT("XXXXXXXXXX"), GOTO(10, 40), SETPEN(),
            PRINT("XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX"), NULL);

        TickitWindow *win =
            tickit_window_new(root, (TickitRect){10, 20, 1, 50}, TICKIT_WINDOW_LOWEST);
        ...
        is_termlog("Termlog for print sibling under floating window", GOTO(10, 40), SETPEN(),
            PRINT("YYYYYYYYYYYYYYYYYYYYYYYYYYYYYY"), NULL);

        TickitWindow *popupwin =
            tickit_window_new(win, (TickitRect){2, 2, 10, 10}, TICKIT_WINDOW_POPUP);
        tickit_window_flush(root);
        ...
        tickit_window_bind_event(
            popupwin, TICKIT_WINDOW_ON_KEY, 0, &on_key_input_capture, &keyinfo);
        ...
        tickit_window_bind_event(
            popupwin, TICKIT_WINDOW_ON_MOUSE, 0, &on_mouse_input_capture, &mouseinfo);
    ...
    // Drawing on floats
    {
        int bind_id = tickit_window_bind_event(
            rootfloat, TICKIT_WINDOW_ON_EXPOSE, 0, &on_expose_textat, "|-- Yipee --|");
        tickit_window_expose(rootfloat, NULL);
        tickit_window_flush(root);

        is_termlog("Termlog for print to floating window", GOTO(10, 10), SETPEN(),
            PRINT("|-- Yipee --|"), NULL);
        ...
        is_termlog("Termlog for print to child of floating window", GOTO(10, 14), SETPEN(),
            PRINT("Byenow"), NULL);
    ...
    {
        int bind_id =
            tickit_window_bind_event(root, TICKIT_WINDOW_ON_EXPOSE, 0, &on_expose_pushrect, NULL);
        tickit_window_flush(root);
        drain_termlog();
        ...
        is_termlog("Termlog after scroll with floats", SETPEN(), SCROLLRECT(0, 0, 10, 80, 3, 0),
            SCROLLRECT(15, 0, 10, 80, 3, 0), NULL);
