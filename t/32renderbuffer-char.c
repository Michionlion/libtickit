...
int main(int argc, char *argv[]) {
    ...
    // Absolute characters
    {
        ...
        is_int(tickit_renderbuffer_get_cell_text(rb, 5, 5, buffer, sizeof buffer), 1,
            "get_cell_text CHAR at 5,5");
        is_str(buffer, "A", "buffer text at 5,5");

        tickit_renderbuffer_flush_to_term(rb, tt);
        is_termlog("RenderBuffer renders char_at to terminal", GOTO(5, 5), SETPEN(.fg = 4),
            PRINT("A"), SETPEN(.fg = 4), PRINT("B"), SETPEN(.fg = 4), PRINT("C"), NULL);
    ...
    // VC characters
    {
        ...
        tickit_renderbuffer_flush_to_term(rb, tt);
        is_termlog(
            "RenderBuffer renders chars at VC", GOTO(0, 4), SETPEN(.fg = 5), PRINT("G"), NULL);
    ...
    // Characters with translation
    {
        ...
        tickit_renderbuffer_flush_to_term(rb, tt);
        is_termlog("RenderBuffer renders char_at with translation", GOTO(4, 6), SETPEN(),
            PRINT("1"), SETPEN(), PRINT("2"), NULL);
    }
