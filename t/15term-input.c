...
int on_key(TickitTerm *tt, TickitEventFlags flags, void *_info, void *data) {
    ...
    keytype = info->type;
    strncpy(keystr, info->str, sizeof(keystr) - 1);
...
int main(int argc, char *argv[]) {
    ...
    is_int(
        tickit_term_input_check_timeout_msec(tt), -1, "term has no timeout after completed Down");
    ...
    is_int(
        tickit_term_input_check_timeout_msec(tt), -1, "term has no timeout after completed Escape");

    {
        ...
        is_int(keytype, TICKIT_KEYEV_TEXT,
            "keytype after push bytes with two events bound the second time");
...

    {
        ...
        int bindB_id =
            tickit_term_bind_event(tt, TICKIT_TERM_ON_KEY, TICKIT_BIND_FIRST, &on_key_push, "B");
