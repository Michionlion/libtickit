...
void is_rect(TickitRect *got, const char *expect, char *name) {
    ...
    if (got->top != exp.top || got->left != exp.left || got->lines != exp.lines ||
        got->cols != exp.cols) {
        fail(name);
        diag("got %d,%d..%d,%d expected %d,%d..%d,%d", got->left, got->top, tickit_rect_right(got),
            tickit_rect_bottom(got), exp.left, exp.top, tickit_rect_right(&exp),
            tickit_rect_bottom(&exp));
    } else {
        pass(name);
    }
}
