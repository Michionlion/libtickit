...
void drain_termlog(void) { tickit_mockterm_clearlog(mt); }
        ...
        switch (exp->type) {
            ...
            case EXPECT_ERASECH:
                if (exp->val[0] == got->val1 && exp->val[1] == got->val2)
                    continue;
                fail(name);
                diag("Expected %s(%d,%d), got %s(%d,%d)", lognames[exp->type], exp->val[0],
                    exp->val[1], lognames[got->type], got->val1, got->val2);
                goto failed;
            ...
            case EXPECT_PRINT:
                if (streq(exp->str, got->str))
                    continue;
                fail(name);
                diag("Expected print(\"%s\"), got print(\"%s\")", exp->str, got->str);
                goto failed;
            ...
            case EXPECT_SCROLLRECT:
                ...
                fail(name);
                diag(
                    "Expected scrollrect(%d,%d,%d,%d, %+d,%+d), got scrollrect(%d,%d,%d,%d, "
                    "%+d,%+d)",
                    exp->val[0], exp->val[1], exp->val[2], exp->val[3], exp->val[4], exp->val[5],
                    got->rect.top, got->rect.left, got->rect.lines, got->rect.cols, got->val1,
                    got->val2);
                goto failed;
