...
struct TIDriver {
    ...
    struct {
        ...
        // Moving
        const char *cuu;
        const char *cuu1;  // Cursor Up
        const char *cud;
        const char *cud1;  // Cursor Down
        const char *cuf;
        const char *cuf1;  // Cursor Forward == Right
        const char *cub;
        const char *cub1;  // Cursor Backward == Left

        // Editing
        const char *ich;
        const char *ich1;  // Insert Character
        const char *dch;
        const char *dch1;  // Delete Character
        const char *il;
        const char *il1;  // Insert Line
        const char *dl;
        const char *dl1;   // Delete Line
        ...
        const char *sm_csr;
        const char *rm_csr;  // Set/reset mode: Cursor visible
    } str;

    const struct TermInfoExtraStrings *extra;
};

static const char *lookup_ti_string(
    struct TIDriver *td, const TickitTermProbeArgs *args, enum unibi_string s) {
...
static const char *require_ti_string(
    struct TIDriver *td, const TickitTermProbeArgs *args, enum unibi_string s) {
...
static bool erasech(TickitTermDriver *ttd, int count, TickitMaybeBool moveend) {
     ...
     */
    if (td->cap.bce &&
        !tickit_pen_get_bool_attr(tickit_termdrv_current_pen(ttd), TICKIT_PEN_REVERSE)) {
...
static bool chpen(TickitTermDriver *ttd, const TickitPen *delta, const TickitPen *final) {
    ...
    int c;
    if ((c = tickit_pen_get_colour_attr(final, TICKIT_PEN_FG)) > -1 && c < td->cap.colours)
        run_ti(ttd, td->str.sgr_fg, 1, c);

    if ((c = tickit_pen_get_colour_attr(final, TICKIT_PEN_BG)) > -1 && c < td->cap.colours)
...
static bool setctl_int(TickitTermDriver *ttd, TickitTermCtl ctl, int value) {
    ...
    switch (ctl) {
        case TICKIT_TERMCTL_ALTSCREEN:
            ...
            tickit_termdrv_write_str(
                ttd, value ? td->extra->enter_altscreen_mode : td->extra->exit_altscreen_mode, 0);
        ...
        case TICKIT_TERMCTL_MOUSE:
            ...
            tickit_termdrv_write_str(
                ttd, value ? td->extra->enter_mouse_mode : td->extra->exit_mouse_mode, 0);
            td->mode.mouse = !!value;
            return true;
...
static TickitTermDriver *new (const TickitTermProbeArgs *args) { return NULL; }
