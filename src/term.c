...
static TermKey *get_termkey(TickitTerm *tt) {
    ...
    termkey_set_canonflags(tt->termkey, termkey_get_canonflags(tt->termkey) | TERMKEY_CANON_DELBS);
...
TickitTerm *tickit_term_new(void) { return tickit_term_build(NULL); }
...
const char *tickit_term_get_termtype(TickitTerm *tt) { return tt->termtype; }

TickitTermDriver *tickit_term_get_driver(TickitTerm *tt) { return tt->driver; }
...
int tickit_term_get_output_fd(const TickitTerm *tt) { return tt->outfd; }
...
int tickit_term_get_input_fd(const TickitTerm *tt) { return tt->infd; }

TickitMaybeBool tickit_term_get_utf8(const TickitTerm *tt) { return tt->is_utf8; }
...
void tickit_term_await_started_tv(TickitTerm *tt, const struct timeval *timeout) {
    ...
    while (tt->state != STARTED) {
        if (!tt->driver->vtable->started || (*tt->driver->vtable->started)(tt->driver))
            break;
...
static void got_key(TickitTerm *tt, TermKey *tk, TermKeyKey *key) {
    if (tt->driver->vtable->gotkey && (*tt->driver->vtable->gotkey)(tt->driver, tk, key))
        return;
    ...
    if (key->type == TERMKEY_TYPE_MOUSE) {
        ...
        switch (ev) {
            case TERMKEY_MOUSE_PRESS:
                info.type = TICKIT_MOUSEEV_PRESS;
                break;
            case TERMKEY_MOUSE_DRAG:
                info.type = TICKIT_MOUSEEV_DRAG;
                break;
            case TERMKEY_MOUSE_RELEASE:
                info.type = TICKIT_MOUSEEV_RELEASE;
                break;
            default:
                info.type = -1;
                break;
...
/* Driver API */
TickitPen *tickit_termdrv_current_pen(TickitTermDriver *ttd) { return ttd->tt->pen; }

void tickit_term_clear(TickitTerm *tt) { (*tt->driver->vtable->clear)(tt->driver); }
