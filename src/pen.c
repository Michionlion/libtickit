...
struct TickitPen {
    ...
    TickitPenRGB8 fg_rgb8, bg_rgb8;

    unsigned int bold : 1, italic : 1, reverse : 1, strike : 1, blink : 1;

    signed int under : 3;   /* 0 to 3 or -1 */
    signed int altfont : 5; /* 1 - 10 or -1 */

    struct {
        unsigned int fgindex : 1, bgindex : 1, fg_rgb8 : 1, bg_rgb8 : 1, bold : 1, under : 1,
            italic : 1, reverse : 1, strike : 1, altfont : 1, blink : 1;
    } valid;
...
static void freeze(TickitPen *pen) { pen->freezecount++; }
...
bool tickit_pen_has_attr(const TickitPen *pen, TickitPenAttr attr) {
    switch (attr) {
        case TICKIT_PEN_FG:
            return pen->valid.fgindex;
        case TICKIT_PEN_BG:
            return pen->valid.bgindex;
        case TICKIT_PEN_BOLD:
            return pen->valid.bold;
        case TICKIT_PEN_UNDER:
            return pen->valid.under;
        case TICKIT_PEN_ITALIC:
            return pen->valid.italic;
        case TICKIT_PEN_REVERSE:
            return pen->valid.reverse;
        case TICKIT_PEN_STRIKE:
            return pen->valid.strike;
        case TICKIT_PEN_ALTFONT:
            return pen->valid.altfont;
        case TICKIT_PEN_BLINK:
            return pen->valid.blink;
...
bool tickit_pen_has_colour_attr_rgb8(const TickitPen *pen, TickitPenAttr attr) {
    switch (attr) {
        case TICKIT_PEN_FG:
            return pen->valid.fgindex && pen->valid.fg_rgb8;
        case TICKIT_PEN_BG:
            return pen->valid.bgindex && pen->valid.bg_rgb8;
        default:
            return 0;
    }
}
...
bool tickit_pen_equiv_attr(const TickitPen *a, const TickitPen *b, TickitPenAttr attr) {
    switch (tickit_pen_attrtype(attr)) {
        ...
        case TICKIT_PENTYPE_COLOUR:
            ...
            if (!tickit_pen_has_colour_attr_rgb8(a, attr) &&
                !tickit_pen_has_colour_attr_rgb8(b, attr))
                return true;
            if (!tickit_pen_has_colour_attr_rgb8(a, attr) ||
                !tickit_pen_has_colour_attr_rgb8(b, attr))
                return false;
        ...
        case TICKIT_PENTYPE_COLOUR:
            ...
            if (tickit_pen_has_colour_attr_rgb8(src, attr))
                tickit_pen_set_colour_attr_rgb8(
                    dst, attr, tickit_pen_get_colour_attr_rgb8(src, attr));
            thaw(dst);
            return;
    }
...
void tickit_pen_copy(TickitPen *dst, const TickitPen *src, bool overwrite) {
    ...
    for (TickitPenAttr attr = 1; attr < TICKIT_N_PEN_ATTRS; attr++) {
        if (!tickit_pen_has_attr(src, attr))
            continue;
        if (tickit_pen_has_attr(dst, attr) && (!overwrite || tickit_pen_equiv_attr(src, dst, attr)))
            continue;
...
TickitPenAttr tickit_pen_lookup_attr(const char *name) {
    switch (name[0]) {
        case 'a':
            return streq(name + 1, "f") ? TICKIT_PEN_ALTFONT : -1;
        case 'b':
            return name[1] == 0
                       ? TICKIT_PEN_BOLD
                       : streq(name + 1, "g") ? TICKIT_PEN_BG
                                              : streq(name + 1, "link") ? TICKIT_PEN_BLINK : -1;
        case 'f':
            return streq(name + 1, "g") ? TICKIT_PEN_FG : -1;
        case 'i':
            return name[1] == 0 ? TICKIT_PEN_ITALIC : -1;
        case 'r':
            return streq(name + 1, "v") ? TICKIT_PEN_REVERSE : -1;
        case 's':
            return streq(name + 1, "trike") ? TICKIT_PEN_STRIKE : -1;
        case 'u':
            return name[1] == 0 ? TICKIT_PEN_UNDER : -1;
    }
    return -1;
}
