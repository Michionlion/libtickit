...
#define WINDOW_PRINTF_ARGS(w)                                                \
    (w)->rect.cols, (w)->rect.lines, tickit_window_get_abs_geometry(w).left, \
        tickit_window_get_abs_geometry(w).top
...
static void _do_hierarchy_change(
    HierarchyChangeType change, TickitWindow *parent, TickitWindow *win);
...
static int on_term_resize(TickitTerm *term, TickitEventFlags flags, void *_info, void *user) {
    ...
    DEBUG_LOGF("Ir", "Resize to %dx%d", info->cols, info->lines);
...
static int on_term_key(TickitTerm *term, TickitEventFlags flags, void *_info, void *user) {
    ...
    DEBUG_LOGF("Ik", "Key event %s %s (mod=%02x)", evnames[info->type], info->str, info->mod);
...
static int on_term_mouse(TickitTerm *term, TickitEventFlags flags, void *_info, void *user) {
    ...
    DEBUG_LOGF("Im", "Mouse event %s %d @%d,%d (mod=%02x)", evnames[info->type], info->button,
        info->col, info->line, info->mod);
    ...
    if (info->type == TICKIT_MOUSEEV_DRAG && root->drag_source_window &&
...
TickitWindow *tickit_window_new_root2(Tickit *t, TickitTerm *term) {
    ...
    init_window(ROOT_AS_WINDOW(root), NULL,
        (TickitRect){.top = 0, .left = 0, .lines = lines, .cols = cols});
    ...
    root->event_ids[0] =
        tickit_term_bind_event(term, TICKIT_TERM_ON_RESIZE, 0, &on_term_resize, root);
    root->event_ids[1] = tickit_term_bind_event(term, TICKIT_TERM_ON_KEY, 0, &on_term_key, root);
    root->event_ids[2] =
        tickit_term_bind_event(term, TICKIT_TERM_ON_MOUSE, 0, &on_term_mouse, root);
...
TickitWindow *tickit_window_new_root(TickitTerm *tt) { return tickit_window_new_root2(NULL, tt); }
...
TickitWindow *tickit_window_new(TickitWindow *parent, TickitRect rect, TickitWindowFlags flags) {
    ...
    _do_hierarchy_change((flags & TICKIT_WINDOW_LOWEST) ? TICKIT_HIERARCHY_INSERT_LAST
                                                        : TICKIT_HIERARCHY_INSERT_FIRST,
        parent, win);
...
TickitWindow *tickit_window_parent(const TickitWindow *win) { return win->parent; }

TickitWindow *tickit_window_root(const TickitWindow *win) { return ROOT_AS_WINDOW(_get_root(win)); }
...
TickitTerm *tickit_window_get_term(const TickitWindow *win) { return _get_root(win)->term; }
...
void tickit_window_destroy(TickitWindow *win) {
    ...
    DEBUG_LOGF("W*", "Window destroyed " WINDOW_PRINTF_FMT, WINDOW_PRINTF_ARGS(win));
...
void tickit_window_show(TickitWindow *win) {
    win->is_visible = true;
    if (win->parent) {
        if (!win->parent->focused_child && (win->focused_child || win->is_focused)) {
...
bool tickit_window_is_visible(TickitWindow *win) { return win->is_visible; }

TickitRect tickit_window_get_geometry(const TickitWindow *win) { return win->rect; }
...
int tickit_window_bottom(const TickitWindow *win) { return tickit_rect_bottom(&win->rect); }

int tickit_window_right(const TickitWindow *win) { return win->rect.left + win->rect.cols; }

void tickit_window_resize(TickitWindow *win, int lines, int cols) {
    tickit_window_set_geometry(win,
        (TickitRect){.top = win->rect.top, .left = win->rect.left, .lines = lines, .cols = cols});
}

void tickit_window_reposition(TickitWindow *win, int top, int left) {
    tickit_window_set_geometry(win,
        (TickitRect){.top = top, .left = left, .lines = win->rect.lines, .cols = win->rect.cols});
...
void tickit_window_set_geometry(TickitWindow *win, TickitRect geom) {
    if ((win->rect.top != geom.top) || (win->rect.left != geom.left) ||
        (win->rect.lines != geom.lines) || (win->rect.cols != geom.cols)) {
...
TickitPen *tickit_window_get_pen(const TickitWindow *win) { return win->pen; }
...
void tickit_window_expose(TickitWindow *win, const TickitRect *exposed) {
    ...
    DEBUG_LOGF("Wd", "Damage root " RECT_PRINTF_FMT, RECT_PRINTF_ARGS(damaged));
...
static void _do_expose(TickitWindow *win, const TickitRect *rect, TickitRenderBuffer *rb) {
    DEBUG_LOGF("Wx", "%sExpose " WINDOW_PRINTF_FMT " " RECT_PRINTF_FMT, _gen_indent(win),
        WINDOW_PRINTF_ARGS(win), RECT_PRINTF_ARGS(*rect));
...
void tickit_window_flush(TickitWindow *win) {
    ...
    if (root->needs_expose) {
        ...
        TickitRenderBuffer *rb =
            tickit_renderbuffer_new(root_window->rect.lines, root_window->rect.cols);
...
static void _do_hierarchy_change(
    ...
    if (fmt)
        DEBUG_LOGF("Wh", fmt, WINDOW_PRINTF_ARGS(parent), WINDOW_PRINTF_ARGS(win));
...
static bool _scrollrectset(
    TickitWindow *win, TickitRectSet *visible, int downward, int rightward, TickitPen *pen) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        DEBUG_LOGF("Wsr", "Term scrollrect " RECT_PRINTF_FMT " by %+d,%+d", RECT_PRINTF_ARGS(rect),
            rightward, downward);
        ...
        if (tickit_term_scrollrect(term, rect, downward, rightward)) {
            if (downward > 0) {
                ...
                tickit_window_expose(
                    origwin, &(TickitRect){.top = tickit_rect_bottom(&origrect) - downward,
                                 .lines         = downward,
                                 .left          = origrect.left,
                                 .cols          = rect.cols});
            } else if (downward < 0) {
            ...
            if (rightward > 0) {
                ...
                tickit_window_expose(origwin, &(TickitRect){
                                                  .top   = origrect.top,
                                                  .lines = rect.lines,
                                                  .left  = tickit_rect_right(&origrect) - rightward,
                                                  .cols  = rightward,
                                              });
            } else if (rightward < 0) {
                tickit_window_expose(origwin, &(TickitRect){
...
static bool _scroll(TickitWindow *win, const TickitRect *origrect, int downward, int rightward,
    TickitPen *pen, bool mask_children) {
    TickitRect rect;
    TickitRect selfrect = {.top = 0, .left = 0, .lines = win->rect.lines, .cols = win->rect.cols};
    ...
    DEBUG_LOGF(
        "Ws", "Scroll " RECT_PRINTF_FMT " by %+d,%+d", RECT_PRINTF_ARGS(rect), rightward, downward);
...
void tickit_window_take_focus(TickitWindow *win) { _focus_gained(win, NULL); }
...
bool tickit_window_is_focused(const TickitWindow *win) { return win->is_focused; }
...
bool tickit_window_is_steal_input(const TickitWindow *win) { return win->steal_input; }
