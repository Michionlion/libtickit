...
static bool evloop_io_read(void *data, int fd, TickitBindFlags flags, TickitWatch *watch) {
    ...
    if (idx == evdata->nfds && evdata->nfds == evdata->alloc_fds) {
        ...
        struct pollfd *newpollfds =
            realloc(evdata->pollfds, sizeof(struct pollfd) * evdata->alloc_fds * 2);
        if (!newpollfds)
            return false;
        TickitWatch **newpollwatches =
            realloc(evdata->pollwatches, sizeof(TickitWatch *) * evdata->alloc_fds * 2);
        if (!newpollwatches)
            return false;
