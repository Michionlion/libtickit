...
void tickit_rectset_clear(TickitRectSet *trs) { trs->count = 0; }

size_t tickit_rectset_rects(const TickitRectSet *trs) { return trs->count; }

size_t tickit_rectset_get_rect(const TickitRectSet *trs, size_t i, TickitRect *rect) {
    if (i >= trs->count)
        return 0;
...
bool tickit_rectset_contains(const TickitRectSet *trs, const TickitRect *rectptr) {
    ...
    for (int i = 0; i < trs->count; i++) {
        ...
        if (rect.top < r->top || rect.left < r->left)
            return false;
