...
static int on_expose(TickitWindow *win, TickitEventFlags flags, void *_info, void *data) {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        if (!colours[i].pen_fg)
            colours[i].pen_fg = tickit_pen_new_attrs(TICKIT_PEN_FG, colours[i].val, 0);
    ...
    for (int i = 0; i < 4; i++) {
        ...
        if (!colours[i].pen_fg_hi)
            colours[i].pen_fg_hi = tickit_pen_new_attrs(TICKIT_PEN_FG, colours[i].val + 8, 0);
    ...
    for (int i = 0; i < 4; i++) {
        ...
        if (!colours[i].pen_bg)
            colours[i].pen_bg =
                tickit_pen_new_attrs(TICKIT_PEN_BG, colours[i].val, TICKIT_PEN_FG, 0, 0);
    ...
    for (int i = 0; i < 4; i++) {
        ...
        if (!colours[i].pen_bg_hi)
            colours[i].pen_bg_hi =
                tickit_pen_new_attrs(TICKIT_PEN_BG, colours[i].val + 8, TICKIT_PEN_FG, 0, 0);
        ...
        if (attrs[i].attr == TICKIT_PEN_UNDER) {
            ...
            for (under = TICKIT_PEN_UNDER_DOUBLE; under < TICKIT_N_PEN_UNDERS; under++) {
                ...
                tickit_renderbuffer_text(
                    rb, (char *[]){NULL, NULL, "double-underline", "undercurl"}[under]);
            }
        }
